...
| 主界面 | 深色主题 |
| --- | --- |
| ![主界面](docs/images/main-light.png) | ![深色主题](docs/images/main-dark.png) |

K 线 / 分时为独立窗口，含分时、日K、周K、月K 四种周期：

<p align="center">
  <img src="docs/images/kline.png" width="640" alt="K 线窗口">
</p>
...
| 桌面浮窗 | 透明置顶小窗，指数在上、个股在下；高度自适应内容，不遮挡下方窗口点击 |
| 价格提醒 | 上破价、下跌价、涨幅、跌幅四类条件；支持暂停 30 分钟；同一只 5 分钟内只提醒一次 |
...
| 开/关提醒 | 股票行尾的铃铛：蓝色为开启、灰色带斜杠为关闭、橙色为已暂停 |
| 展开提醒条件 | 股票行尾的 `›` 箭头 |
...
- 自测：`npm test`（打包配置校验 + 静态审计 + 行情与调度单测 + DOM 交互测试）
