...
| 模块 | 一句话 |
| --- | --- |
| 自选 | 代码 / 名称 / 拼音联想添加，支持股票 / ETF / LOF / 可转债 |
| 指数 | 14 个内置指数，勾选启停 |
| 浮窗 | 透明置顶，指数在上个股在下，可贴边收起成小球 |
| 提醒 | 价格 / 涨跌幅共四种条件 |
| 通道 | 桌面通知 + 声音 |
| 外观 | 明暗主题 + 定制滚动条 |
| 常驻 | 托盘图标 + 全局热键 |
...
- **品种识别**：ETF / LOF / 可转债带类型标签，基金与可转债按 3 位小数显示，不会误当股票
- **三通道容错**：东财、新浪、腾讯并行取数、按只合并，一家挂了另外两家自动补上
- **代理友好**：请求走 Chromium 网络栈，自动吃系统代理与 PAC，公司网也能用
- **浮窗自适应**：高度贴合内容，不留透明死区挡住下方点击；拖到屏幕边缘自动贴边收成小球
- **提醒不轰炸**：同一只股票 5 分钟内只提醒一次，支持暂停 30 分钟
- **诊断说人话**：状态灯一点即知数据来自哪条通道、走的是直连还是代理、哪个通道为什么失败
- **拖拽排序**：按住自选行首 `⠿` 直接拖
...
| 隐藏浮窗 | `Esc` |
| 浮窗收起/展开 | 拖到屏幕左右边缘自动收成小球，鼠标移入展开；双击手动切换 |
| 开关提醒 | 股票行尾的 🔔 |
| 展开提醒条件 | 股票行尾的 `›` |
...
- [x] **ETF / 场内基金适配** — 区分股票 / ETF / LOF / 可转债，按品种显示小数位
- [x] **浮窗贴边吸附** — 拖到屏幕边缘自动吸附，左右侧收起成小球，鼠标移入展开
- [x] **检查更新** — 应用内检查 Releases 新版本并提示下载
- [x] **代理设置** — 跟随系统代理 / 手动代理，解决公司网络拦截行情
- [x] **自选拖拽排序** — 按住 `⠿` 拖动调整顺序
...
- [ ] **基金折溢价** — ETF / LOF 显示 IOPV 与折溢价率

### 长期

- [ ] **绿色版数据本地化** — 便携版把配置写在 exe 同级目录，U 盘可用
- [ ] **自动更新** — 接入 `electron-updater` 静默增量更新（当前是提示后手动下载）
