...
checkDom('alert', alertHtml, alertJs);

// preload 暴露面 ↔ 业务代码调用面
const exposed = new Set([...preload.matchAll(/^\s{2}([a-zA-Z]\w*):/gm)].map((m) => m[1]));
for (const [label, js] of [['renderer', appJs], ['float', floatJs], ['chart', chartJs], ['alert', alertJs]]) {
